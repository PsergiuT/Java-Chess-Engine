...
public class FenTranslator {
    ...
    public static void translate(String fen, BitBoard board){
        //r3k2r/8/8/8/8/8/8/R3K1R1 w Qkq - 0 1
        //rnbqkp - black pieces
        //RNBQKP - white pieces
        //Qkq - white can caslte on white queen, black can castle on black queen and king
        //starting rank = 8
        // / = new rank
        // number = spaces
        ...
        long whitePawnBoard = 0, whiteKnightBoard = 0, whiteBishopBoard = 0, whiteRookBoard = 0, whiteQueenBoard = 0, whiteKingBoard = 0;
        long blackPawnBoard = 0, blackKnightBoard = 0, blackBishopBoard = 0, blackRookBoard = 0, blackQueenBoard = 0, blackKingBoard = 0;
        ...
        for(int i = 0; i < 8; i++){
            ...
            while(position < 8){
                if(boardSplit[i].charAt(index) >= '0' && boardSplit[i].charAt(index) <= '9'){
                    position += boardSplit[i].charAt(index) - '0';
                }
                else{
                    if(boardSplit[i].charAt(index) >= 'a' && boardSplit[i].charAt(index) <= 'z'){
                        //black piece
                        switch(boardSplit[i].charAt(index)){
                            case 'p': blackPawnBoard |= (1L << ((7-position) + (7-i)*8)); break;
                            case 'n': blackKnightBoard |= (1L << ((7-position) + (7-i)*8)); break;
                            case 'b': blackBishopBoard |= (1L << ((7-position) + (7-i)*8)); break;
                            case 'r': blackRookBoard |= (1L << ((7-position) + (7-i)*8)); break;
                            case 'q': blackQueenBoard |= (1L << ((7-position) + (7-i)*8)); break;
                            case 'k': blackKingBoard |= (1L << ((7-position) + (7-i)*8)); break;
                        }
                    }
                    else{
                        //white piece
                        switch(boardSplit[i].charAt(index)){
                            case 'P': whitePawnBoard |= (1L << ((7-position) + (7-i)*8)); break;
                            case 'N': whiteKnightBoard |= (1L << ((7-position) + (7-i)*8)); break;
                            case 'B': whiteBishopBoard |= (1L << ((7-position) + (7-i)*8)); break;
                            case 'R': whiteRookBoard |= (1L << ((7-position) + (7-i)*8)); break;
                            case 'Q': whiteQueenBoard |= (1L << ((7-position) + (7-i)*8)); break;
                            case 'K': whiteKingBoard |= (1L << ((7-position) + (7-i)*8)); break;
                        }
                    }
                    position ++;
                }
        ...
        board.setWhitePawnBoard(whitePawnBoard);
        board.setWhiteKnightBoard(whiteKnightBoard);
        board.setWhiteBishopBoard(whiteBishopBoard);
        board.setWhiteRookBoard(whiteRookBoard);
        board.setWhiteQueenBoard(whiteQueenBoard);
        board.setWhiteKingBoard(whiteKingBoard);
        board.setBlackPawnBoard(blackPawnBoard);
        board.setBlackKnightBoard(blackKnightBoard);
        board.setBlackBishopBoard(blackBishopBoard);
        board.setBlackRookBoard(blackRookBoard);
        board.setBlackQueenBoard(blackQueenBoard);
        board.setBlackKingBoard(blackKingBoard);

        //set color to move
        board.setIsWhiteTurn(fenSplit[1].equals("w"));

        //set castling rights
        board.setWhiteKingCastle(false);
        board.setBlackKingCastle(false);
        board.setWhiteQueenCastle(false);
        board.setBlackQueenCastle(false);

        if(fenSplit[2].charAt(0) != '-'){
            for(int i = 0; i < fenSplit[2].length(); i++){
                switch(fenSplit[2].charAt(i)){
                    case 'K': board.setWhiteKingCastle(true); break;
                    case 'Q': board.setWhiteQueenCastle(true); break;
                    case 'k': board.setBlackKingCastle(true); break;
                    case 'q': board.setBlackQueenCastle(true); break;
                }
            }
        }
        ...
        if(fenSplit[3].charAt(0) != '-'){
            int file = 'h' - fenSplit[3].charAt(0);
            int rank = Integer.parseInt(fenSplit[3].substring(1)) - 1;
            board.setEnPassantSquare(file + rank*8);
        }
        else{
    ...
    public static final String[] FENS = {
            ...
            "r3k2r/8/8/8/8/8/8/R3K2R w KQkq - 0 1",             //13
            ...
            "4k3/8/8/8/8/8/8/4K2R b K - 0 1",               //20th
