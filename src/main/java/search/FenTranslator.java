...
public class FenTranslator {
    ...
    public static void translate(String fen, BitBoard board){
        ...
        //rnbqkp - black pieces
        //RNBQKP - white pieces
        //Qkq - white can castle on white queen, black can castle on black queen and king
        //starting rank = 8
        // / = new rank
        // number = spaces
        ...
        for(int i = 0; i < 8; i++){
            ...
            while(position < 8){
                if(boardSplit[i].charAt(index) >= '0' && boardSplit[i].charAt(index) <= '9'){
                    ...
                    long l = 1L << ((7 - position) + (7 - i) * 8);
                    if(boardSplit[i].charAt(index) >= 'a' && boardSplit[i].charAt(index) <= 'z'){
                        //black piece
                        switch(boardSplit[i].charAt(index)){
                            case 'p': blackPawnBoard |= l; break;
                            case 'n': blackKnightBoard |= l; break;
                            case 'b': blackBishopBoard |= l; break;
                            case 'r': blackRookBoard |= l; break;
                            case 'q': blackQueenBoard |= l; break;
                            case 'k': blackKingBoard |= l; break;
                        }
                    }
                    else{
                        //white piece
                        switch(boardSplit[i].charAt(index)){
                            case 'P': whitePawnBoard |= l; break;
                            case 'N': whiteKnightBoard |= l; break;
                            case 'B': whiteBishopBoard |= l; break;
                            case 'R': whiteRookBoard |= l; break;
                            case 'Q': whiteQueenBoard |= l; break;
                            case 'K': whiteKingBoard |= l; break;
                        }
                    }
                    position ++;
                }
